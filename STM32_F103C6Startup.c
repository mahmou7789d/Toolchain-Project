...
void Reset_Handler() 
{
    ...
    //in casd we make this pointer uint32_t type , process will be faster but not safe in case you didn't apply align(4)
    uint8_t* SrcPtr = (uint8_t*)&_etext; 
    /*Start of .data section in SRAM*/
    uint8_t* DecPtr = (uint8_t*)&_sdata;

    uint32_t i;

    for(i=0;i<size;i++) 
    {
        *DecPtr = *SrcPtr;  //**DecPtr++ = *SrcPtr++
        DecPtr++;
        SrcPtr++;
    }
	/* 2- initalize .bss section in SRAM to zero */
    size = (uint32_t)&_ebss-(uint32_t)&_sbss;

    DecPtr = (uint8_t*)&_sbss;
